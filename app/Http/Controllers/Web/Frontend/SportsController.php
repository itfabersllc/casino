...
namespace VanguardLTE\Http\Controllers\Web\Frontend {
    ...
    class SportsController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        {
            $games = \VanguardLTE\Game::where('status', 1)->offset(0)->take(50)->get();
            return json_decode($games);
        }

        public function get_provider()
        {
            $categories = \VanguardLTE\GameCategory::select(
                ...
                ->leftJoin('games', 'game_categories.game_id', '=', 'games.id')
                ->where('games.status', 1)
                ->groupBy('game_categories.category_id', 'categories.title', 'categories.href')
                ->get();
        ...
        {
            if ($request->id == 'all') {
                $games = \VanguardLTE\Game::where('status', 1)->offset($request->page * 12)->take(12)->get();
                return json_decode($games);
            } else {
                ...
                if (count($categoryId)) {
                    $games = \VanguardLTE\Game::whereIn('id', $categoryId)->where('status', 1)->offset($request->page * 12)->take(12)->get();
                } else {
                    $games = \VanguardLTE\Game::where('id', 0)->where('status', 1)->offset($request->page * 12)->take(12)->get();
                }
                return json_decode($games);
            }
        }
