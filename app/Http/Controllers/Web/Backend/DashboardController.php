...
namespace VanguardLTE\Http\Controllers\Web\Backend
{
    ...
    class DashboardController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function __construct(\VanguardLTE\Repositories\User\UserRepository $users, \VanguardLTE\Repositories\Activity\ActivityRepository $activities, \Illuminate\Http\Request $request)
        {
...
//            }
        ...
        {
            $banks = \VanguardLTE\GameBank::where('shop_id', auth()->user()->shop_id);
            if($banks->count() == 0)
            {
        ...
        public function generator(\Illuminate\Http\Request $request)
        {
            ...
            $games = \VanguardLTE\Game::where(['shop_id', auth()->user()->shop_id,
                'status' => 1])->get()->pluck('name');
