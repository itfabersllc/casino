...
namespace VanguardLTE\Http\Controllers\Web\Backend
{
    ...
    class GamesController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function index(\Illuminate\Http\Request $request)
        {
            ...
            $games = \VanguardLTE\Game::select('games1.*')->where('shop_id', auth()->user()->shop_id);
            if($games->count() == 0)
            {
                $g_table = (new \VanguardLTE\Game)->getTable();
                \DB::statement('insert '.\DB::getTablePrefix().$g_table.'(name,title, shop_id, jpg_id,label, device,gamebank,chanceFirepot1,chanceFirepot2,chanceFirepot3,fireCount1,fireCount2,fireCount3,lines_percent_config_spin, lines_percent_config_spin_bonus, lines_percent_config_bonus,lines_percent_config_bonus_bonus, rezerv,cask,advanced,bet, scaleMode, slotViewState, view,denomination,category_temp, original_id, bids,stat_in, stat_out) (select name, title, '.auth()->user()->shop_id.' as shop_id, jpg_id, label, device,gamebank,chanceFirepot1,chanceFirepot2,chanceFirepot3,fireCount1,fireCount2,fireCount3,lines_percent_config_spin, lines_percent_config_spin_bonus, lines_percent_config_bonus,lines_percent_config_bonus_bonus, rezerv,cask,advanced,bet, scaleMode, slotViewState, view,denomination,category_temp, original_id, bids,stat_in, stat_out from '.\DB::getTablePrefix().$g_table.' where shop_id=0)');
                $games = \VanguardLTE\Game::select('games1.*')->where('shop_id', auth()->user()->shop_id);
            ...
            if( $savedCategory ) 
            {
                $games = $games->join('game_categories', 'game_categories.game_id', '=', 'games1.original_id');
            ...
            $allGames = \VanguardLTE\Game::select('games1.*')->where('shop_id', auth()->user()->shop_id)->get();
