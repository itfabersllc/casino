<?php
namespace VanguardLTE\Games\LadyJesterGT
{
    class SlotSettings
    {
...
//end slotsounds update
        ...
        public function CalculateSpin($bet, $lines, $linesId, $cReels)
        {
            $totalWin = 0;
            $minWin = 0;
            $fLoop = true;
            for( $lp = 0; $lp <= 5; $lp++ )
            {
                ...
                $wild = ['P_1'];
                $scatter = 'SCAT';
                $reels = $this->GetReelStrips('', 'freespin');
                for( $r = 1; $r <= 5; $r++ )
                {
                    $isWild = false;
                    for( $p = 0; $p <= 3; $p++ )
                    {
                        if( $cReels['reel' . $r][$p] == 'P_1' )
                        {
                            $isWild = true;
                            break;
                        }
                ...
                for( $k = 0; $k < $lines; $k++ )
                {
                    for( $j = 0; $j < count($this->SymbolGame); $j++ )
                    {
                        ...
                        if( $csym == $scatter || $csym == 'P_1_1' || $csym == 'P_1_2' || $csym == 'P_1_3' || $csym == 'P_1_4' )
                        {
                        }
                        else
                        {
                            ...
                            {
                                $mpl = 1;
                                $tmpWin = $this->Paytable[$csym][1] * $bet * $mpl;
                                if( $cWins[$k] < $tmpWin )
                                {
                                    $cWins[$k] = $tmpWin;
                                }
                            ...
                            {
                                $mpl = 1;
                                $tmpWin = $this->Paytable[$csym][2] * $bet * $mpl;
                                if( $cWins[$k] < $tmpWin )
                                {
                                    $cWins[$k] = $tmpWin;
                                }
                            ...
                            {
                                $mpl = 1;
                                $tmpWin = $this->Paytable[$csym][3] * $bet * $mpl;
                                if( $cWins[$k] < $tmpWin )
                                {
                                    $cWins[$k] = $tmpWin;
                                }
                            ...
                            {
                                $mpl = 1;
                                $tmpWin = $this->Paytable[$csym][4] * $bet * $mpl;
                                if( $cWins[$k] < $tmpWin )
                                {
                                    $cWins[$k] = $tmpWin;
                                }
                            ...
                            {
                                $mpl = 1;
                                $tmpWin = $this->Paytable[$csym][5] * $bet * $mpl;
                                if( $cWins[$k] < $tmpWin )
                                {
                                    $cWins[$k] = $tmpWin;
                                }
        ...
        public function UpdateJackpots($bet, $isJack, $isJack2)
        {
            ...
            for( $jp = 1; $jp <= 1; $jp++ )
            {
                if( $this->jpgs[$jp - 1]->balance != '' )
                {
                    ...
                    else if( $count_balance < $bet && !$isJack )
                    {
                        $slotJackSum[$jp - 1] = $count_balance / 100 * $this->slotJackPercent[$jp - 1] + $this->slotJackpot[$jp - 1];
                    }
                    else if( !$isJack )
                    {
                    ...
                    if( $this->jpgs[$jp - 1]->get_pay_sum() <= $this->slotJackpot[$jp - 1] && !$isJackPay && $this->jpgs[$jp - 1]->get_pay_sum() > 0 && $isJack2 && $isJack )
                    {
                        ...
                        else
                        {
                            $isJackPay = 0;
                            $isJackId = $jp - 1;
                            $this->Jackpots['jackPay'] = $slotJackSum[$jp - 1];
                            $this->Jackpots['jackType'] = 'slotType1';
                            $leftOver = 0;
                        }
                    }
                    if( $this->jpgs[$jp - 1]->get_pay_sum() <= $this->slotJackpot[$jp - 1] && !$isJackPay && $this->jpgs[$jp - 1]->get_pay_sum() > 0 && !$isJack2 && $isJack )
        ...
        public function PutBonusToLine($rpID, $pos, $sid)
        {
            $rp = $this->{'reelStrip' . $rpID};
            $rpResult = [];
            for( $i = 1; $i < (count($rp) - 5); $i++ )
            {
                if( $rp[$i] == $sid && $pos == 1 )
                {
