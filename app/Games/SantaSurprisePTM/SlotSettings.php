<?php
namespace VanguardLTE\Games\SantaSurprisePTM
{
    class SlotSettings
    {
        ...
        public function SaveLogReport($spinSymbols, $bet, $lines, $win, $slotState)
        {
            ...
            else if( $slotState == 'prize_bonus' )
            {
                $reportName = $this->slotId . ' BG';
            }
        ...
        public function EggBonus($bet, $eggBonusNum)
        {
            ...
            for( $j = 0; $j < 1500; $j++ )
            {
                ...
                for( $i = 0; $i < 6; $i++ )
                {
                    $curValues[$i] = $eggPrizes[$i];
                    if( $i < $eggBonusNum )
                    {
                        $totalWin += ($eggPrizes[$i] * $bet);
                    }
                }
                if( $totalWin <= $bank )
                {
        ...
        public function GetRandomScatterPos($rp, $randomBonus)
        {
            $rpResult = [];
            for( $i = 0; $i < count($rp); $i++ )
            {
                if( $rp[$i] == $randomBonus[0] )
                {
            ...
            else
            {
                ...
                for( $i = 0; $i < count($reelsId); $i++ )
                {
                    if( $i < $scattersCnt )
                    {
                        if( $randomBonus[0] == '9' )
                        {
                            $prs[$reelsId[$i]] = $this->PutBonusToLine($this->{'reelStrip' . $reelsId[$i]}, $linesId[$randomLine][$i]);
                        }
                        else
                        {
                            $prs[$reelsId[$i]] = $this->GetRandomScatterPos($this->{'reelStrip' . $reelsId[$i]}, $randomBonus);
                        }
                    }
                    else
                    {
