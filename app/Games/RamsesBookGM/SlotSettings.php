<?php
namespace VanguardLTE\Games\RamsesBookGM
{
    class SlotSettings
    {
        ...
        public function GetSpinSettings($garantType = 'bet', $bet, $lines)
        {
            ...
            else if( $this->GetGameDataStatic('RtpControlCount') < 0 )
            {
                ...
                if( $this->GetGameDataStatic('RtpControlCount') < (-1 * ($RtpControlCount * 4)) && $currentPercent - 1 <= $rtpRange && $rtpRange <= ($currentPercent + 2) )
                {
                    $this->SetGameDataStatic('RtpControlCount', $RtpControlCount);
                }
            }
