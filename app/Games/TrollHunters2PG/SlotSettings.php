...
namespace VanguardLTE\Games\TrollHunters2PG
{
    class SlotSettings
    {
        ...
        public function SymbolsOffset($reels)
        {
            $newReels = [];
            $newReels['reel1'] = [];
            $newReels['reel2'] = [];
            ...
            for( $r = 1; $r <= 5; $r++ )
            {
                for( $p = count($newReels['reel' . $r]) + 1; $p <= 5; $p++ )
                {
                    array_unshift($newReels['reel' . $r], -1);
                }
            }
            return $newReels;
        }
        ...
        public function GetReelStrips($winType, $slotEvent, $bonusPosition)
        {
            $game = $this->game;
            if( $winType != 'bonus' )
            {
