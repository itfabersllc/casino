...
namespace VanguardLTE\Games\HotGemsPT
{
    class SlotSettings
    {
        ...
        public function OffsetReels($reels, $slotEvent)
        {
            $game = $this->game;
            ...
            for( $r = 1; $r <= 5; $r++ )
            {
                ...
                {
                    $offset = $reels['rp'][$r - 1] - 2;
                    if( $offset < 0 )
                    {
                        $offset = count($this->{'reelStrip' . $r}) - 2;
                    }
                    $reels['rp'][$r - 1] -= 1;
                    $reels['reel' . $r][0] = $this->{'reelStrip' . $r}[$offset];
                }
                if( $reels['reel' . $r][0] != '-1' && $reels['reel' . $r][1] == '-1' && $reels['reel' . $r][2] != '-1' )
                {
                ...
                if( $reels['reel' . $r][0] == '-1' && $reels['reel' . $r][1] == '-1' && $reels['reel' . $r][2] != '-1' )
                {
                    $offset0 = $reels['rp'][$r - 1] - 2;
                    if( $offset0 < 0 )
                    {
                        $offset0 = count($this->{'reelStrip' . $r}) - 2;
                    }
                    $offset1 = $reels['rp'][$r - 1] - 3;
                    if( $offset1 < 0 )
                    {
                        $offset1 = count($this->{'reelStrip' . $r}) - 3;
                    }
                    $reels['rp'][$r - 1] -= 2;
                    $reels['reel' . $r][1] = $this->{'reelStrip' . $r}[$offset0];
                    $reels['reel' . $r][0] = $this->{'reelStrip' . $r}[$offset1];
                }
                if( $reels['reel' . $r][0] != '-1' && $reels['reel' . $r][1] == '-1' && $reels['reel' . $r][2] == '-1' )
                {
                ...
                if( $reels['reel' . $r][0] == '-1' && $reels['reel' . $r][1] == '-1' && $reels['reel' . $r][2] == '-1' )
                {
                    ...
                    $offset2 = $reels['rp'][$r - 1] - 4;
                    if( $offset2 < 0 )
                    {
                        $offset2 = count($this->{'reelStrip' . $r}) - 4;
                    }
                    $reels['rp'][$r - 1] -= 3;
                    $reels['reel' . $r][2] = $this->{'reelStrip' . $r}[$offset0];
                    $reels['reel' . $r][1] = $this->{'reelStrip' . $r}[$offset1];
                    $reels['reel' . $r][0] = $this->{'reelStrip' . $r}[$offset2];
                }
                if( $reels['rp'][$r - 1] < 0 )
                {
                    $reels['rp'][$r - 1] = count($this->{'reelStrip' . $r}) - 1 - ($reels['rp'][$r - 1] * -1);
                }
            }
            return $reels;
        }
        public function GetReelsWin($reels, $lines, $betLine, $linesId, $cWins, $mpl)
        {
            $wild = ['0'];
            $scatter = '13';
            $totalWin = 0;
            ...
            for( $k = 0; $k < $lines; $k++ )
            {
                ...
                for( $j = 0; $j < count($this->SymbolGame); $j++ )
                {
                    ...
                    else
                    {
                        ...
                        {
                            $tmpWin = $this->Paytable['SYM_' . $csym][2] * $betLine * $mpl;
                            if( $cWins[$k] < $tmpWin )
                            {
                                $cWins[$k] = $tmpWin;
                                $reelsOffset['reel1'][$linesId[$k][0] - 1] = '-1';
                                $reelsOffset['reel2'][$linesId[$k][1] - 1] = '-1';
                            }
                        }
                        if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) )
                        {
                            $tmpWin = $this->Paytable['SYM_' . $csym][3] * $betLine * $mpl;
                            if( $cWins[$k] < $tmpWin )
                            {
                                $cWins[$k] = $tmpWin;
                                $reelsOffset['reel1'][$linesId[$k][0] - 1] = '-1';
                                $reelsOffset['reel2'][$linesId[$k][1] - 1] = '-1';
                                $reelsOffset['reel3'][$linesId[$k][2] - 1] = '-1';
                            }
                        }
                        if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) && ($s[3] == $csym || in_array($s[3], $wild)) )
                        {
                            $tmpWin = $this->Paytable['SYM_' . $csym][4] * $betLine * $mpl;
                            if( $cWins[$k] < $tmpWin )
                            {
                                $cWins[$k] = $tmpWin;
                                $reelsOffset['reel1'][$linesId[$k][0] - 1] = '-1';
                                $reelsOffset['reel2'][$linesId[$k][1] - 1] = '-1';
                                $reelsOffset['reel3'][$linesId[$k][2] - 1] = '-1';
                                $reelsOffset['reel4'][$linesId[$k][3] - 1] = '-1';
                            }
                        }
                        if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) && ($s[3] == $csym || in_array($s[3], $wild)) && ($s[4] == $csym || in_array($s[4], $wild)) )
                        {
                            $tmpWin = $this->Paytable['SYM_' . $csym][5] * $betLine * $mpl;
                            if( $cWins[$k] < $tmpWin )
                            {
                                $cWins[$k] = $tmpWin;
                                $reelsOffset['reel1'][$linesId[$k][0] - 1] = '-1';
                                ...
                                $reelsOffset['reel3'][$linesId[$k][2] - 1] = '-1';
                                $reelsOffset['reel4'][$linesId[$k][3] - 1] = '-1';
                                $reelsOffset['reel5'][$linesId[$k][4] - 1] = '-1';
                            }
                        }
                    }
                }
            ...
            for( $r = 1; $r <= 5; $r++ )
            {
                for( $p = 0; $p <= 2; $p++ )
                {
                    if( $reels['reel' . $r][$p] == '12' && $scattersCount >= 2 )
                    {
                        $reelsOffset['reel' . $r][$p] = '-1';
                    }
                    if( $reels['reel' . $r][$p] == '13' && $scattersCount2 >= 3 )
                    {
                        $reelsOffset['reel' . $r][$p] = '-1';
                    }
                }
            }
            $scattersWin = $this->Paytable['SYM_12'][$scattersCount] * $betLine * $lines * $mpl;
