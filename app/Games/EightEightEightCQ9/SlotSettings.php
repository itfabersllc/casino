...
namespace VanguardLTE\Games\EightEightEightCQ9
{
    class SlotSettings
    {
        ...
        {
            $rpResult = [];
            for( $i = 2; $i < (count($rp) - 1); $i++ )
            {
                if( $rp[$i] == $bs || $rp[$i - 1] == $bs || $rp[$i - 2] == $bs )
                {
        ...
        public function GetReelStrips($winType, $slotEvent)
        {
            ...
            if( $winType != 'bonus' )
            {
                ...
                ] as $index => $reelStrip )
                {
                    ...
                    {
                        $prs[$index + 1] = mt_rand(2, count($this->$reelStrip) - 1);
                    }
                }
            }
            else
            {
                ...
                for( $i = 0; $i < count($reelsId); $i++ )
                {
                    if( $i == 1 || $i == 2 || $i == 3 )
                    {
                        $prs[$reelsId[$i]] = $this->GetRandomScatterPos($this->{'reelStrip' . $reelsId[$i]}, $bs[0]);
                    }
                    else
                    {
                        $prs[$reelsId[$i]] = rand(2, count($this->{'reelStrip' . $reelsId[$i]}) - 1);
                    }
                }
            }
            $reel = [
