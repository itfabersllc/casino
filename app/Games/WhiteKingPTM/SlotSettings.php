<?php
namespace VanguardLTE\Games\WhiteKingPTM
{
    class SlotSettings
    {
        ...
        public function GetRandomScatterPos($rp)
        {
            ...
            for( $i = 0; $i < count($rp); $i++ )
            {
                if( $rp[$i] == '11' )
                {
                    if( isset($rp[$i + 1]) && isset($rp[$i + 2]) && isset($rp[$i - 1]) )
                    {
        ...
        public function GetReelStrips($winType, $slotEvent)
        {
            ...
            else
            {
                ...
                $prs[1] = mt_rand(0, count($this->reelStrip1) - 3);
                $prs[2] = $this->GetRandomScatterPos($this->reelStrip2);
                $prs[3] = $this->GetRandomScatterPos($this->reelStrip3);
                $prs[4] = $this->GetRandomScatterPos($this->reelStrip4);
                $prs[5] = mt_rand(0, count($this->reelStrip5) - 3);
            }
            $reel = [
                'rp' => []
            ];
