<?php
namespace VanguardLTE\Games\TempleOfSecretsGTM
{
    class SlotSettings
    {
...
//end slotsounds update
        ...
        public function UpdateJackpots($bet, $jid, $isBet)
        {
            ...
            for( $jp = 1; $jp <= 10; $jp++ )
            {
                if( $this->jpgs[$jp - 1]->balance != '' )
                {
                    ...
                    else if( $count_balance < $bet && !$isBet )
                    {
                    ...
                    if( $this->jpgs[$jp - 1]->get_pay_sum() <= $this->slotJackpot[$jp - 1] && !$isJackPay && $this->jpgs[$jp - 1]->get_pay_sum() > 0 && $jid == ($jp - 1) )
                    {
