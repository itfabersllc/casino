...
namespace VanguardLTE\Games\MonopolyMegawaysBGT
{
    class SlotSettings
    {
        ...
        public function OffsetReels($reels)
        {
            ...
            $newReels['reel6'] = [];
            $fixReels = [];
            $fixReels[1] = $this->reelStrip1;
            $fixReels[2] = $this->reelStrip2;
            $fixReels[3] = $this->reelStrip3;
            $fixReels[4] = $this->reelStrip4;
            $fixReels[5] = $this->reelStrip5;
            $fixReels[6] = $this->reelStrip6;
            $rp = [
                0,
                0,
                0,
            ...
            for( $r = 1; $r <= 6; $r++ )
            {
                ...
                $rp[$r - 1] = rand(1, count($this->{'reelStrip' . $r}) - 8);
                $cr = $rp[$r - 1] + (7 - (count($newReels['reel' . $r]) + 1));
                for( $p = count($newReels['reel' . $r]) + 1; $p <= 7; $p++ )
                {
                    array_unshift($newReels['reel' . $r], $this->{'reelStrip' . $r}[$cr + $rpc]);
                    $rpc--;
                }
            }
            $newReels['rp'] = $rp;
        ...
        public function FormatWins($reelsWin)
        {
            $resultWinStr = [];
            $resultWinStr_ = [];
            $resultWinCnt = 0;
            for( $k = 1; $k <= 7; $k++ )
            {
                ...
                {
                    $isNewSym = false;
                    for( $j = $reelsWin['rc' . $i]; $j < 7; $j++ )
                    {
                        if( $reelsWin['reel' . $i][$j] != -1 )
                        {
                            $t_s[] = $j . ';';
                            $isNewSym = true;
                            $reelsWin['rc' . $i]++;
                        ...
                        {
                            $t_s[] = $reelsWin['reel' . $i][$j] . ';';
                        }
                    }
                }
                $resultWinStr[] = $resultWinCnt . ';1;' . implode('', $t_s);
                $resultWinStr_[] = '1';
                $resultWinCnt++;
            }
            array_pop($resultWinStr_);
            return [
                implode('|', $resultWinStr),
                implode('|', $resultWinStr_)
            ];
        }
                    ...
                    for( $r = 1; $r <= 6; $r++ )
                    {
                        ...
                        for( $p = 0; $p <= 6; $p++ )
                        {
                            if( $reels['reel' . $r][$p] == $csym || $reels['reel' . $r][$p] == $wild )
                            {
                                $isEmpty = false;
                                $symCounts[$r]++;
                                if( $this->Paytable['SYM_' . $csym][$r] > 0 )
                                {
                                    $lastReel = $r;
                                }
                            }
            ...
            foreach( $prs as $index => $value )
            {
                ...
                for( $pw = 0; $pw < 7; $pw++ )
                {
                    if( $pw < $rpwc )
                    {
                        if( $index == 3 )
                        {
                            $reel['reel' . $index][$pw] = $key[$value + $pw];
                        }
                        else
                        {
