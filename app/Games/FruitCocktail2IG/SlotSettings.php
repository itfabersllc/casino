...
namespace VanguardLTE\Games\FruitCocktail2IG
{
    class SlotSettings
    {
        ...
        public function GetFreeSpins($bet, $lines, $countFreeSpins)
        {
            $linesId = [];
            $linesId[1] = [
                2,
            ...
            $freeSeq = [];
            $allFreeWin = 0;
            for( $floop = $countFreeSpins - 1; $floop >= 0; $floop-- )
            {
                $isBonus = false;
                $isWin = false;
                $i = 0;
                ...
                {
                    $totalWin = 0;
                    $reels = $this->GetReelStrips(false, false, 'free');
                    $lineWins = [
                        0,
                        0,
                        0,
                    ...
                    for( $lp = 0; $lp <= 4; $lp++ )
                    {
                        ...
                        if( $reels['reel' . ($lp + 1)][0] == 9 )
                        {
                            $bonusSym2++;
                        }
                        if( $reels['reel' . ($lp + 1)][1] == 9 )
                        {
                            $bonusSym2++;
                        }
                        if( $reels['reel' . ($lp + 1)][2] == 9 )
                        {
                            $bonusSym2++;
                        }
                    }
                ...
                $allFreeWin += $totalWin;
                $freeSeq[] = '{"FreeGamesCnt":' . $floop . ',"LineWins":' . json_encode($lineWinsInfo) . ',"Reels":[' . $reelStr . '],"TotalBet":' . ($bet * $lines) . ',"Win":' . $totalWin . '}';
            }
            return [
                $allFreeWin,
                $freeSeq
        ...
        public function GetRandomScatterPos($rp, $winType, $winType2)
        {
            if( $winType2 )
            {
                $bss = '9';
        ...
        public function GetReelStrips($winType, $winType2, $status = '')
        {
            $game = $this->game;
            if( $status == 'free' )
            {
            ...
            else
            {
                ...
                for( $i = 0; $i < count($reelsId); $i++ )
                {
                    if( $i < $scattersCnt )
                    {
                        $prs[$reelsId[$i]] = $this->GetRandomScatterPos($this->{'reelStrip' . $reelsId[$i]}, $winType, $winType2);
                    }
                    else
                    {
