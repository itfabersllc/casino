<?php
namespace VanguardLTE\Games\WildRespinAM
{
    class SlotSettings
    {
        ...
        public function GetRandomScatterPos($rp, $sym, $full)
        {
            $rpResult = [];
            for( $i = 1; $i < (count($rp) - 4); $i++ )
            {
                if( $full )
                {
                    if( $rp[$i] == $sym && $rp[$i + 1] == $sym && $rp[$i - 1] == $sym && $rp[$i + 2] == $sym )
                    {
            ...
            else
            {
                ...
                for( $i = 0; $i < count($reelsId); $i++ )
                {
                    if( $i == 0 || $i == $rSym )
                    {
                        if( $i == 0 )
                        {
                            $prs[$reelsId[$i]] = $this->GetRandomScatterPos($this->{'reelStrip' . $reelsId[$i]}, $rSym0, true);
                        }
                        else
                        {
                            $prs[$reelsId[$i]] = $this->GetRandomScatterPos($this->{'reelStrip' . $reelsId[$i]}, '9', false);
                        }
                    }
                    else
                    {
