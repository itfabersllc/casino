<?php
namespace VanguardLTE\Games\AlchemistsSecretGTM
{
    class SlotSettings
    {
...
//end slotsounds update
        ...
        public function GetBonusPreset($bet, $lines)
        {
            global $playerId;
            global $dblink;
            $bank = $this->Bank / $bet;
            $pr = \VanguardLTE\BonusPreset::whereRaw('win <= ? and game = ? and game_lines = ?', [
                $bank,
                $this->slotId,
                $lines
            ])->get();
        ...
        public function GetHistory()
        {
            ...
            foreach( $history as $log )
            {
                ...
                $tmpLog_ = json_decode($log->str);
                if( $tmpLog->responseEvent != 'gambleResult' )
                {
                    $tmpLog_->serverResponse->FreeSeries = '';
                    $this->lastEvent = json_encode($tmpLog_);
                    break;
                }
            }
            if( isset($tmpLog) )
        ...
        public function UpdateJackpots($bet, $isJack)
        {
            ...
            for( $jp = 1; $jp <= 1; $jp++ )
            {
                if( $this->jpgs[$jp - 1]->balance != '' )
                {
                    ...
                    if( $this->jpgs[$jp - 1]->get_pay_sum() <= $this->slotJackpot[$jp - 1] && !$isJackPay && $this->jpgs[$jp - 1]->get_pay_sum() > 0 && $isJack )
                    {
