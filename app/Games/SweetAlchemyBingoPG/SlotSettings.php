...
namespace VanguardLTE\Games\SweetAlchemyBingoPG
{
    class SlotSettings
    {
        ...
        public function CheckPattern(&$patternArr, $card, $num)
        {
            $curPos = 0;
            ...
            for( $i = 0; $i < count($patternArr); $i++ )
            {
                ...
                for( $j = count($patternArr[$i]) - 1; $j >= 2; $j-- )
                {
                    if( $patternArr[$i][$j] != -1 )
                    {
                        $isPay = false;
        ...
        public function CalculateBonus($cardNum)
        {
            ...
            $otherItems = [];
            for( $i = count($resultArr); $i < (count($resultArr) + 25); $i++ )
            {
                $otherItems[] = $winItems[$i];
            }
            $pay = $pay * $mpl;
            $result = '2 2 15 0 ' . count($resultArr) . ' ' . implode(' ', $resultArr) . ' ' . implode(' ', $otherItems) . '';
            return [
                $result,
                $pay
            ];
