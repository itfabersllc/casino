<?php
namespace VanguardLTE\Games\PirateIG
{
    class SlotSettings
    {
        ...
        public function Bonus2($bet, $fuseBet)
        {
            $bonusWins = [
                1,
                1,
            ...
            {
                $randomDice = rand(1, 6);
                $currentBar += $randomDice;
                if( $currentBar > 10 )
                {
                    $currentBar = $currentBar - 10;
                }
                $cwin = $bonusWins[$i] * $bet * $barrels[$currentBar];
                $barrels[$currentBar] = 0;
                $bWin += $cwin;
                if( $cwin > 0 )
                {
                    $gem = ',"Gem":' . $gems[$bonusWins[$i]];
                }
                else
                {
                    $gem = '';
                }
                $bStr[] = '{"Coef":' . $bonusWins[$i] . ',"Win":' . $cwin . ',"Die":' . $randomDice . '' . $gem . '}';
                if( $cwin == 0 )
                {
                    break;
                }
            }
            $strOut = '"Bonus1Wins":[[' . implode(',', $bStr) . ']],';
            return [
                'win' => $bWin,
                'info' => $strOut
            ];
        }
        public function Bonus($bet, $fuseBet, $bSymCnt)
        {
            $bWin = 0;
            $bStr = [];
            $strOut = '';
            ...
            for( $i = 0; $i < $bSymCnt; $i++ )
            {
                ...
                $bWin += $cwin;
                $bStr[] = '{"Coef":' . $bonusWins[$i] . ',"Win":' . $cwin . ',"Die":0}';
                if( $cwin == 0 )
                {
                    break;
                }
        ...
        public function GetReelStrips($winType, $tmpBsCnt = 3)
        {
            if( !$winType )
            {
                $prs = [];
