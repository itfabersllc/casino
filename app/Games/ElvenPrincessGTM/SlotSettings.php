<?php
namespace VanguardLTE\Games\ElvenPrincessGTM
{
    class SlotSettings
    {
...
//end slotsounds update
        ...
        public function UpdateJackpots($bet, $jid)
        {
            ...
            for( $jp = 1; $jp <= 2; $jp++ )
            {
                if( $this->jpgs[$jp - 1]->balance != '' )
                {
                    ...
                    if( $count_balance == 0 && !$jid )
                    {
                        $slotJackSum[$jp - 1] = $this->slotJackpot[$jp - 1];
                    }
                    else if( !$jid )
                    {
                        if( $count_balance < $bet )
                        {
                    ...
                    if( $this->jpgs[$jp - 1]->get_pay_sum() <= $this->slotJackpot[$jp - 1] && !$isJackPay && $this->jpgs[$jp - 1]->get_pay_sum() > 0 && $jp == 2 && !$jid )
                    {
                    ...
                    if( $this->jpgs[$jp - 1]->get_pay_sum() <= $this->slotJackpot[$jp - 1] && !$isJackPay && $this->jpgs[$jp - 1]->get_pay_sum() > 0 && $jp == 1 && $jid )
                    {
                        if( $this->jpgs[$jp - 1]->user_id && $this->jpgs[$jp - 1]->user_id != $this->user->id )
                        {
                        }
