        ...
        public function UpdateJackpots($bet, $isJack)
        {
            ...
            for( $jp = 1; $jp <= 2; $jp++ )
            {
                if( $this->jpgs[$jp - 1]->balance != '' )
                {
                    ...
                    if( ($count_balance == 0 || $this->jpgPercentZero) && !$isJack )
                    {
                        $slotJackSum[$jp - 1] = $this->slotJackpot[$jp - 1];
                    }
                    else if( !$isJack )
                    ...
                    if( $this->jpgs[$jp - 1]->get_pay_sum() <= $this->slotJackpot[$jp - 1] && !$isJackPay && $this->jpgs[$jp - 1]->get_pay_sum() > 0 && $isJack && $jp == 1 )
                    {
                    ...
                    if( $this->jpgs[$jp - 1]->get_pay_sum() <= $this->slotJackpot[$jp - 1] && !$isJackPay && $this->jpgs[$jp - 1]->get_pay_sum() > 0 && !$isJack && $jp == 2 )
                    {
                        if( $this->jpgs[$jp - 1]->user_id && $this->jpgs[$jp - 1]->user_id != $this->user->id )
                        {
                        }
                        else
                        {
                            ...
                            $leftOver = 0;
                            $this->Jackpots['jackPay'] = $slotJackSum[$jp - 1];
                            $this->Jackpots['jackType'] = 'slotType2';
                        }
                    }
