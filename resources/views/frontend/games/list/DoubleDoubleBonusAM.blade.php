...
document.cookie = 'PHPSESSID=; Max-Age=0; path=/; domain=' + location.host;
 ...
 window.onerror=function(){return true};
    ...
    if( !sessionStorage.getItem('sessionId') ){
		...
		document.location.href=document.location.href+'/?hash=&curr=@if( auth()->user()->present()->shop ){{ auth()->user()->present()->shop->currency }}@endif&lang=ru&uselang=en&exit=&config=1372';	
		}else if(document.location.href.split("?api_exit")[1]!=undefined){
			
		document.location.href=uparts[0]+'/?hash=&curr=@if( auth()->user()->present()->shop ){{ auth()->user()->present()->shop->currency }}@endif&lang=ru&uselang=en&exit=&config=1372&'+uparts[1];	
		}
...
<body onload="injectCSS()"  style="display:flex; justify-content: center;  align-items: center;">
	...
	<script type="text/javascript" src="./src/doubledoublebonusloader_00267472.js"></script>
</body>
</html>