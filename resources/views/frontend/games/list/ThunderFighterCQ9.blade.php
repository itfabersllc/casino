...
<base href="/games/{{ $game->name }}/AT04/">
    <link rel="icon" type="image/x-icon" href="res/_favicon.ico?v=1" />
...
.gamebg {
   background-image: url(res/img/img_pc_bg.jpg);
...
<body onload="injectCSS()"  style="display:flex;    justify-content: center;     align-items: center;padding:0; margin: 0;">
  <div class="gamebg"></div>
<canvas id="gameCanvas" width="600" height="960" style="z-index: -1"></canvas>
<script>
...
document.location.href=exitUrl;	
    ...
    var dollarsign = true;
		var GameName='ThunderFighterCQ9';
    if(url.searchParams.has('language'))
    {
        var param = url.searchParams.get('language');
        if(param == 'zh-cn')
            document.getElementById('title').innerHTML = '雷电战机';
        else
            document.getElementById('title').innerHTML = 'ThunderFighter';
    }else{
        document.getElementById('title').innerHTML = 'ThunderFighter';
    }
