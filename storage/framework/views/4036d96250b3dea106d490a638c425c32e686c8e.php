<script type="text/javascript">
    ...
    $('body').on('click', '#daily_entry', function(event){
        ...
        $.ajax({
            url: "<?php echo e(route('frontend.profile.daily_entry')); ?>",
            type: "GET",
            dataType: "json",
    ...
    $('body').on('click', '#sendContactForm', function(event){
        ...
        $.ajax({
            url: "<?php echo e(route('frontend.profile.contact')); ?>",
            type: "POST",
    ...
    //                 popup.find('.popup__value').attr('data-attr', parseFloat(data.value));
    //                 popup.find('.popup__value').html(parseFloat(data.value));
    //                 popup.find('.popup__type').html('<?php echo app('translator')->get('app.refunds'); ?>');
    ...
    //         $.getJSON('<?php echo e(route('frontend.profile.ajax')); ?>', function(data) {
    //             if( data.jackpots ){
    ...
    // setInterval(function(){
    //     $.getJSON('<?php echo e(route('frontend.profile.message')); ?>', function(data) {
    ...
    $(document).on('keyup', '.search', function() {
        ...
        doSearch(query, '<?php echo e($category1); ?>');
    });
    ...
    */
    ...
    function doSearch(query, category1=''){
        $.getJSON('<?php echo e(route('frontend.search.json')); ?>?q=' + query + '&search_all_db=1&category1=' + category1, function(data) {
            $('#games').html('');
            if( data.success ){
                var elems = data.data;
...
</script><?php /**PATH /var/www/casino/resources/views/frontend/Default/partials/scripts.blade.php ENDPATH**/ ?>