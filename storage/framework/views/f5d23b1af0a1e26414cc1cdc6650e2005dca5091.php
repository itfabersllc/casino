...
<head>
	...
	<base href="/games/XingYunJinChanSW/">
<title><?php echo e($game->title); ?></title>
	...
	if(location.href.split("?").length > 1) {
		...
		paramsArray.forEach(function(param) {
			if(param.split("=")[0] === "externalFeatures") {
				isWushiJp = param.indexOf("wuShiJp") !== -1;
			}
		});
	}
	if(isWushiJp) {
		...
		Array.from(document.head.children).forEach(item => {
			if(item.rel && (item.rel.indexOf("icon") != -1 || item.rel.indexOf("manifest") != -1)) {
				var href = "assets" + item.href.split('assets')[1];
				href = href.replace("/favicon/", "/faviconWushijp/");
				href = href.replace("/favicon-", "/favicon");
				item.href = href;
			}
		});
	}
...
<body>
	...
	! function(e, n, t) {
		...
		var c = "6.2.3";
		i("wrapper/games/sw_jqw.json", o, function() {
...
<?php /**PATH C:\OSPanel\domains\cas.loc\resources\views/frontend/games/list/XingYunJinChanSW.blade.php ENDPATH**/ ?>